...
-(NSMutableArray * )getSectionDataListWithIndexPath:(NSIndexPath *) indexPath{
    if ([self.dataSourceDelegate respondsToSelector:@selector(cellDataList)]) {
        NSMutableArray * list = [self.dataSourceDelegate cellDataList];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    ...
    }else if([self.dataSourceDelegate respondsToSelector:@selector(cellDataList)]){
        NSArray * list = [self.dataSourceDelegate cellDataList];
        if (list.count > 0 && [[list firstObject] isKindOfClass:[NSArray class]]) {
            return list.count;
...
-(void) openHorizontalAniamtion{
    ...
    self.hidden = true;
    [self performSelector:@selector(startHorizontalOneByOneAnimation) withObject:self afterDelay:0];
}

-(void) openVerticalAniamtion{
    //[self startHorizontalOneByOneAnimation];
    self.hidden = true;
    [self performSelector:@selector(startVerticalOneByOneAnimation) withObject:self afterDelay:0];
}
