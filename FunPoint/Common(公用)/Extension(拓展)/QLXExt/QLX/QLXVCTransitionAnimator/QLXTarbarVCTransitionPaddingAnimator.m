...
@interface QLXTarbarVCTransitionPaddingAnimator ()
...
-(void)interactiveBeginWithProgress:(CGFloat)progress{
    ...
    CGPoint offset = self.panGR.locationInTargetView;
    self.isNextPage = offset.x < 0;
    assert(offset.x != 0);
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
  ...
  //  CGPoint tt = [self.panGR translationInView:self.destinationController.view];
    
    CGPoint cur =  [touch locationInView:self.destinationController.view];
    CGPoint last = [touch previousLocationInView:self.destinationController.view];
    if (fabs(cur.x - last.x) < fabs(cur.y - last.y) || (cur.y > (self.destinationController.view.height -49))) {
        return false;
    }
    if (self.animating) {
        return false;
...
-(QLXPanGestureRecognizer *)panGR{
    if (!_panGR) {
        ...
        _panGR.delayBeginToDirection = true;
